...
class NotificationCenterVC : BaseViewController {
    ...
    func updateNavBar(){
        ...
        let titleWidth = ("Notifications".localized as NSString).size(withAttributes: [NSAttributedString.Key.font: mageFont.mediumFont(size: 15)]).width//width calculate
    ...
    func loadData() {
        notifHandler?.getNotificationCenterData(page: page){ [weak self] json in
            ...
            else if  json["success"].stringValue == "true" && json["data"]["send"].arrayValue.count == 0 && self?.page == 1{
                self?.notificationsData = []
                DispatchQueue.main.async {
                    self?.view.stopLoader()
