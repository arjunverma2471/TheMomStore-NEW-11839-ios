...
open class SweetAlert: UIViewController {
  ...
  fileprivate func setupContentView() {
      contentView.backgroundColor = UIColor(light: UIColor(white: 1.0, alpha: 1.0),dark: UIColor.black)
    contentView.layer.cornerRadius = 12.0
    contentView.layer.masksToBounds = true
    contentView.layer.borderWidth = 0.5
    contentView.addSubview(titleLabel)
    contentView.addSubview(subTitleTextView)
    contentView.backgroundColor = UIColor(light: UIColor.colorFromRGB(0xFFFFFF),dark: UIColor.black)
  ...
  fileprivate func setupTitleLabel() {
    ...
    titleLabel.textAlignment = .center
      titleLabel.font = mageFont.boldFont(size: 16)//UIFont(name: kFont, size:16)
    titleLabel.textColor = UIColor.colorFromRGB(0x797979)//UIColor.colorFromRGB(0x575757)
  }
  
  fileprivate func setupSubtitleTextView() {
    subTitleTextView.text = ""
    subTitleTextView.textAlignment = .center
    subTitleTextView.font = mageFont.regularFont(size: 12)//UIFont(name: kFont, size:12)
  ...
  fileprivate func resizeAndRelayout() {
    ...
    let width: CGFloat = kContentWidth - (kWidthMargin*2) + 20
    ...
    if self.titleLabel.text != nil {
      titleLabel.frame = CGRect(x: x-10, y: y, width: width, height: kTitleHeight)
      contentView.addSubview(titleLabel)
    ...
    if self.subTitleTextView.text.isEmpty == false {
      ...
      textViewHeight = ceil(rect.size.height) + 40.0
      subTitleTextView.frame = CGRect(x: x-10, y: y, width: width, height: textViewHeight)
      contentView.addSubview(subTitleTextView)
    ...
    if buttons.count == 2 {
      totalWidth = buttonRect[0].size.width + buttonRect[1].size.width + kWidthMargin + 120.0
    }
    else{
      totalWidth = buttonRect[0].size.width + 80.0
    }
    y += kHeightMargin
    var buttonX = (kContentWidth - totalWidth ) / 2.0
    for i in 0 ..< buttons.count {
      
      buttons[i].frame = CGRect(x: buttonX, y: y, width: buttonRect[i].size.width + 60.0, height: buttonRect[i].size.height + 20.0)
      buttonX = buttons[i].frame.origin.x + kWidthMargin + buttonRect[i].size.width + 60.0
      buttons[i].layer.cornerRadius = 2.0
    ...
    y += kHeightMargin + buttonRect[0].size.height + 25.0
    if y > kMaxHeight {
